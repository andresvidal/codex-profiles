...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  const windowProfileState = new WindowProfileState(
    vscode.Uri.joinPath(context.globalStorageUri, 'window-state').fsPath,
    context.logUri.toString(),
  );
  const activeProfile = await windowProfileState.resolveActiveProfile(
